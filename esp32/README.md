...
The T-Display LCD already owns the board's default VSPI pins, so the
`ttgo-tdisplay` variant places the MCP2515 on a separate HSPI bus:
...
| MOSI / SI | 25 | |
| INT | (unused) | |
| VCC | 5 V (`5V` pin on T-Display) for TJA1050-based modules; 3V3 only if your module supports it | |
| GND | GND | |
...
The built-in ST7789 display is enabled by default and can be toggled
